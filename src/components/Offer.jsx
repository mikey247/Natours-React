//
import classes from "./Offer.module.css";

const Offer = () => {
  return (
    <div className={classes.offerContainer}>
      <div className={classes.offerWrapper}>
        <h1>WHAT DO WE BRING TO THE TABLE?😏</h1>
        <p>
          We provide amazing experiences that will last you a lifetime.Doesn't
          matter if it's single trips, couples trips, family or group trips we
          are the best choice for you and you wont have to break the bank to get
          the time of your life, Come over to the Natours side of life💚.
        </p>
        <button>LEARN MORE</button>
      </div>
    </div>
  );
};

export default Offer;
